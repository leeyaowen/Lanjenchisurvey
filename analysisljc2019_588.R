...
dttype<-"valley"
dtraw<-read.csv("./All_0816.csv",stringsAsFactors = FALSE)
...
# codt<-matrix(data = c(rep(1997,214),rep(2005,214),rep(2013,214),rep(2019,214)))
# resmodel5<-stimodels(df,S=214,Ti=4,COD.T = codt)
...
dtspeciesnum<-dtstemsp %>%
  summarise(.,sp97=sum(stem97>0),sp05=sum(stem05>0),sp13=sum(stem13>0),sp18=sum(stem18>0))
dtstemfamily<-dtb0 %>%
  group_by(.,科名) %>%
  summarise(.,stem97=sum(live97),stem05=sum(live05),stem13=sum(live13),stem18=sum(live18)) %>%
  filter(.,stem97>0 | stem05>0 | stem13>0 | stem18>0)
dtfamilynum<-dtstemfamily %>%
  summarise(.,family97=sum(stem97>0),family05=sum(stem05>0),family13=sum(stem13>0),family18=sum(stem18>0))
dtstemgenus<-dtb0 %>%
  group_by(.,Genus) %>%
  summarise(.,stem97=sum(live97),stem05=sum(live05),stem13=sum(live13),stem18=sum(live18)) %>%
  filter(.,stem97>0 | stem05>0 | stem13>0 | stem18>0)
dtgenusnum<-dtstemgenus %>%
  summarise(.,genus97=sum(stem97>0),genus05=sum(stem05>0),genus13=sum(stem13>0),genus18=sum(stem18>0))


#生物多樣性指數
diver<-data.frame(t(dtstemsp),stringsAsFactors = F)
colnames(diver)<-diver[1,]
diveryear=c("1997","2005","2013","2019")
diversimpson<-c(indexsimpson97=diversity(as.numeric(diver[-c(1,3:5),]),index = "simpson"),
                indexsimpson05=diversity(as.numeric(diver[-c(1:2,4:5),]),index = "simpson"),
                indexsimpson13=diversity(as.numeric(diver[-c(1:3,5),]),index = "simpson"),
                indexsimpson18=diversity(as.numeric(diver[-c(1:4),]),index = "simpson"))
divershannon<-c(indexshannon97=diversity(as.numeric(diver[-c(1,3:5),]),index = "shannon"),
                indexshannon05=diversity(as.numeric(diver[-c(1:2,4:5),]),index = "shannon"),
                indexshannon13=diversity(as.numeric(diver[-c(1:3,5),]),index = "shannon"),
                indexshannon18=diversity(as.numeric(diver[-c(1:4),]),index = "shannon"))
...
survivestem<-dtb0 %>%
  mutate(.,survive2005=ifelse(live97==1 & live05==1,1,0),
            survive2013=ifelse(live05==1 & live13==1,1,0),
            survive2018=ifelse(live13==1 & live18==1,1,0))
newyearrange<-c("1997-2005","2005-2013","2013-2019")
newratedt<-c(newrate(sum(survivestem$live05),sum(survivestem$survive2005),8),
             newrate(sum(survivestem$live13),sum(survivestem$survive2013),8),
             newrate(sum(survivestem$live18),sum(survivestem$survive2018),6))
...
survivestem<-dtb0 %>%
  mutate(.,survive2005=ifelse(live97==1 & live05==1,1,0),
         survive2013=ifelse(live05==1 & live13==1,1,0),
         survive2018=ifelse(live13==1 & live18==1,1,0))
deadyearrange<-c("1997-2005","2005-2013","2013-2019")
deadratemdt<-c(deadratem(sum(survivestem$survive2005),sum(survivestem$live97),8),
               deadratem(sum(survivestem$survive2013),sum(survivestem$live05),8),
               deadratem(sum(survivestem$survive2018),sum(survivestem$live13),6))
deadratemdf<-t(data.frame(deadyearrange,"年死亡率(m)"=deadratemdt,row.names = 1,check.names=F))
deadratemdf

deadratelambdadt<-c(deadratelambda(sum(survivestem$survive2005),sum(survivestem$live97),8),
               deadratelambda(sum(survivestem$survive2013),sum(survivestem$live05),8),
               deadratelambda(sum(survivestem$survive2018),sum(survivestem$live13),6))
...
changeyearrange<-c("1997-2005","2005-2013","2013-2019")
changeratedt<-c(changerate(sum(dtb0$live97),sum(dtb0$live05),8),
...
#胸高斷面積總和
bayear<-c("1997","2005","2013","2019")
basumdt<-c(sum(dtb0$sumba97),sum(dtb0$sumba05),sum(dtb0$sumba13),sum(dtb0$sumba18))
...
dtbaspha<-dtb0 %>%
  group_by(.,sp) %>%
  summarise(.,allba97=sum(sumba97)/plotha,allba05=sum(sumba05)/plotha,allba13=sum(sumba13)/plotha,allba18=sum(sumba18)/plotha) %>%
  ungroup(.) %>%
  arrange(.,sp)

#IVI
iv<-bind_cols(dtstemspha,dtbaspha,rdtstemspha,rdtbasp) %>%
  mutate(.,iv97=(rstem97+ba97)/2,iv05=(rstem05+ba05)/2,iv13=(rstem13+ba13)/2,iv19=(rstem18+ba18)/2) %>%
  select(.,-c(6,11:20)) %>%
  left_join(.,splist,by=c("sp"="中名")) %>%
  select(.,1,17,2:13)
...
dtdeadbaha<-dtb0 %>%
  ...
  summarise(.,deadba05=sum(dba05)/plotha,deadba13=sum(dba13)/plotha,deadba18=sum(dba18)/plotha)
...
#名錄
kdt<-read.csv("./Result20190816.csv",stringsAsFactors = F)
